...
# Script para desplegar AWS SRA GuardDuty Solution
# Uso: ./deploy-cDeployGuardDutySolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-guardduty-org-solution"
TEMPLATE_FILE="aws_sra/solutions/guardduty/guardduty_org/templates/sra-guardduty-org-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de GuardDuty
print_info "Extrayendo parámetros de GuardDuty..."
...
# Extraer parámetros de GuardDuty
GUARDDUTY_PARAMS=$(extract_section_params "rGuardDutySolutionStack")

if [ -z "$GUARDDUTY_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de GuardDuty del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$GUARDDUTY_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
GUARDDUTY_PARAMS=$(echo "$GUARDDUTY_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de GuardDuty extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-guardduty-org-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de GuardDuty Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $GUARDDUTY_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de GuardDuty completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de GuardDuty implementada:"
    echo "  ✅ GuardDuty habilitado en toda la organización"
    echo "  ✅ Administración delegada configurada"
    echo "  ✅ Clave KMS creada en la cuenta de auditoría"
    echo "  ✅ Bucket S3 creado en la cuenta de archivo de logs"
    echo "  ✅ Configuración de detección:"
    echo "    - S3 Logs habilitados"
    echo "    - Kubernetes Audit Logs habilitados"
    echo "    - Malware Protection habilitado"
    echo "    - RDS Login Events habilitados"
    echo "    - Runtime Monitoring habilitado"
    echo "    - EKS Add-on Management habilitado"
    echo "    - ECS Fargate Agent Management habilitado"
    echo "    - EC2 Agent Management habilitado"
    echo "    - Lambda Network Logs habilitados"
    echo "  ✅ Frecuencia de publicación: FIFTEEN_MINUTES"
    ...
    echo ""
    print_info "Para verificar la configuración de GuardDuty:"
    echo "aws guardduty list-detectors --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws guardduty get-detector --detector-id <detector-id> --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws guardduty list-findings --detector-id <detector-id> --region $AWS_REGION --profile $AWS_PROFILE"
