...
# Script para desplegar AWS SRA CloudTrail Solution
# Uso: ./deploy-cDeployCloudTrailSolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-cloudtrail-org-solution"
TEMPLATE_FILE="aws_sra/solutions/cloudtrail/cloudtrail_org/templates/sra-cloudtrail-org-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de CloudTrail
print_info "Extrayendo parámetros de CloudTrail..."
...
# Extraer parámetros de CloudTrail
CLOUDTRAIL_PARAMS=$(extract_section_params "rCloudTrailSolutionStack")

if [ -z "$CLOUDTRAIL_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de CloudTrail del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$CLOUDTRAIL_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
CLOUDTRAIL_PARAMS=$(echo "$CLOUDTRAIL_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de CloudTrail extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-cloudtrail-org-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de CloudTrail Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $CLOUDTRAIL_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de CloudTrail completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de CloudTrail implementada:"
    echo "  ✅ CloudTrail habilitado en toda la organización"
    echo "  ✅ Clave KMS creada en la cuenta de auditoría"
    echo "  ✅ Bucket S3 creado en la cuenta de archivo de logs"
    echo "  ✅ Eventos de datos configurados:"
    echo "    - Eventos de Lambda habilitados"
    echo "    - Eventos de S3 habilitados"
    echo "    - Solo eventos de datos habilitados"
    echo "  ✅ CloudWatch Log Group configurado"
    echo "  ✅ Retención de logs: 400 días"
    ...
    echo ""
    print_info "Para verificar la configuración de CloudTrail:"
    echo "aws cloudtrail describe-trails --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws cloudtrail get-trail-status --name sra-org-trail --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws logs describe-log-groups --log-group-name-prefix /aws/cloudtrail --region $AWS_REGION --profile $AWS_PROFILE"
