...
# Script para desplegar AWS SRA EC2 Default EBS Encryption Solution
# Uso: ./deploy-cDeployEC2DefaultEBSEncryptionSolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-ec2-default-ebs-encryption-solution"
TEMPLATE_FILE="aws_sra/solutions/ec2/ec2_default_ebs_encryption/templates/sra-ec2-default-ebs-encryption-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de EC2 Default EBS Encryption
print_info "Extrayendo parámetros de EC2 Default EBS Encryption..."
...
# Extraer parámetros de EC2 Default EBS Encryption
EC2_EBS_ENCRYPTION_PARAMS=$(extract_section_params "rEC2DefaultEBSEncryptionSolutionStack")

if [ -z "$EC2_EBS_ENCRYPTION_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de EC2 Default EBS Encryption del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$EC2_EBS_ENCRYPTION_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
EC2_EBS_ENCRYPTION_PARAMS=$(echo "$EC2_EBS_ENCRYPTION_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de EC2 Default EBS Encryption extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-ec2-default-ebs-encryption-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de EC2 Default EBS Encryption Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $EC2_EBS_ENCRYPTION_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de EC2 Default EBS Encryption completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de EC2 Default EBS Encryption implementada:"
    echo "  ✅ Encriptación EBS habilitada por defecto en todas las cuentas"
    ...
    echo ""
    print_info "Para verificar la configuración de EBS:"
    echo "aws ec2 get-ebs-default-kms-key-id --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws ec2 get-ebs-encryption-by-default --region $AWS_REGION --profile $AWS_PROFILE"
