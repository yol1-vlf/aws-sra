...
ACCOUNTS = {
    'master': {
        'profile': 'yol1-vlf-master-admin',
        'name': 'Master Account'
    },
    'audit': {
        'profile': 'yol1-vlf-audit-admin', 
        'name': 'Audit Account'
    },
    'log': {
        'profile': 'yol1-vlf-log-admin',
...
def list_cloudformation_stacks(session, account_name, delete_mode=False):
    ...
    for region in SRA_REGIONS:
        try:
            ...
            logger.error(f"Error al listar stacks en {account_name} ({region}): {e}")
    ...
    if delete_mode:
        for stack_info in all_sra_stacks:
            try:
                ...
                logger.error(f"Error al eliminar stack {stack_name} en {region}: {e}")
...
def list_cloudwatch_log_groups(session, account_name, delete_mode=False):
    ...
    for region in SRA_REGIONS:
        try:
            ...
            logger.error(f"Error al listar grupos de logs en {account_name} ({region}): {e}")
    ...
    if delete_mode:
        for log_group_info in all_sra_log_groups:
            try:
                ...
                logger.error(f"Error al eliminar grupo de logs {log_group_name} en {region}: {e}")
...
def list_ssm_parameters(session, account_name, delete_mode=False):
    ...
    for region in SRA_REGIONS:
        try:
            ...
            logger.error(f"Error al listar parámetros SSM en {account_name} ({region}): {e}")
        ...
        for region, parameters in parameters_by_region.items():
            ...
            for i in range(0, len(parameters), 10):
                ...
                try:
                    ...
                    logger.error(f"Error al eliminar lote de parámetros en {region}: {e}")
        ...
        if delete_mode:
            for role_name in sra_roles:
                try:
                    ...
                    logger.error(f"Error al eliminar rol {role_name}: {e}")
                
    except ClientError as e:
        logger.error(f"Error al listar roles IAM en {account_name}: {e}")
...
def list_lambda_functions(session, account_name, delete_mode=False):
    ...
    for region in SRA_REGIONS:
        try:
            ...
            logger.error(f"Error al listar funciones Lambda en {account_name} ({region}): {e}")
    ...
    if delete_mode:
        for function_info in all_sra_functions:
            try:
                ...
                logger.error(f"Error al eliminar función Lambda {function_name} en {region}: {e}")
        ...
        if delete_mode:
            for bucket_name in sra_buckets:
                try:
                    ...
                    logger.error(f"Error al eliminar bucket {bucket_name}: {e}")
                
    except ClientError as e:
        logger.error(f"Error al listar buckets S3 en {account_name}: {e}")
        ...
        if delete_mode:
            for stackset_name in sra_stacksets:
                try:
                    ...
                    if instances_to_delete:
                        ...
                        for i in range(0, len(instances_to_delete), 10):
                            ...
                            try:
                                ...
                                logger.error(f"Error al eliminar instancias del StackSet {stackset_name}: {e}")
                    ...
                    logger.error(f"Error al eliminar StackSet {stackset_name}: {e}")
                
    except ClientError as e:
        logger.error(f"Error al listar StackSets en {account_name}: {e}")
...
def scan_account(account_key, account_config, delete_mode=False):
    ...
    try:
        ...
        logger.error(f"Error durante la {action_text} de {account_config['name']}: {e}")
        return False
