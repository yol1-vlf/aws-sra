...
# Script para desplegar AWS SRA Account Alternate Contacts Solution
# Uso: ./deploy-cDeployAccountAlternateContactsSolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-account-alternate-contacts-solution"
TEMPLATE_FILE="aws_sra/solutions/account/account_alternate_contacts/templates/sra-account-alternate-contacts-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de Account Alternate Contacts
print_info "Extrayendo parámetros de Account Alternate Contacts..."
...
# Extraer parámetros de Account Alternate Contacts
ACCOUNT_ALTERNATE_CONTACTS_PARAMS=$(extract_section_params "rAccountAlternateContactsSolutionStack")

if [ -z "$ACCOUNT_ALTERNATE_CONTACTS_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de Account Alternate Contacts del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$ACCOUNT_ALTERNATE_CONTACTS_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
ACCOUNT_ALTERNATE_CONTACTS_PARAMS=$(echo "$ACCOUNT_ALTERNATE_CONTACTS_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de Account Alternate Contacts extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-account-alternate-contacts-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de Account Alternate Contacts Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $ACCOUNT_ALTERNATE_CONTACTS_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de Account Alternate Contacts completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de Account Alternate Contacts implementada:"
    echo "  ✅ Contactos alternativos configurados en toda la organización"
    echo "  ✅ Roles IAM configurados para la gestión"
    echo "  ✅ Configuración de contactos:"
    echo "    - Billing: $pBillingName ($pBillingEmail)"
    echo "    - Operations: $pOperationsName ($pOperationsEmail)"
    echo "    - Security: $pSecurityName ($pSecurityEmail)"
    ...
    echo "  ✅ Exclusión por tags configurada"
    ...
    echo ""
    print_info "Para verificar los contactos alternativos:"
    echo "aws account get-alternate-contact --alternate-contact-type BILLING --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws account get-alternate-contact --alternate-contact-type OPERATIONS --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws account get-alternate-contact --alternate-contact-type SECURITY --region $AWS_REGION --profile $AWS_PROFILE"
