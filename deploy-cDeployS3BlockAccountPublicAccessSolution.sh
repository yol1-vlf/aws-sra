...
# Script para desplegar AWS SRA S3 Block Account Public Access Solution
# Uso: ./deploy-cDeployS3BlockAccountPublicAccessSolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-s3-block-account-public-access-solution"
TEMPLATE_FILE="aws_sra/solutions/s3/s3_block_account_public_access/templates/sra-s3-block-account-public-access-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de S3 Block Account Public Access
print_info "Extrayendo parámetros de S3 Block Account Public Access..."
...
# Extraer parámetros de S3 Block Account Public Access
S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS=$(extract_section_params "rS3BlockAccountPublicAccessSolutionStack")

if [ -z "$S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de S3 Block Account Public Access del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS=$(echo "$S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de S3 Block Account Public Access extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-s3-block-account-public-access-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de S3 Block Account Public Access Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $S3_BLOCK_ACCOUNT_PUBLIC_ACCESS_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de S3 Block Account Public Access completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de S3 Block Account Public Access implementada:"
    echo "  ✅ Bloqueo de ACLs públicas configurado"
    echo "  ✅ Bloqueo de políticas públicas configurado"
    echo "  ✅ Ignorar ACLs públicas configurado"
    echo "  ✅ Restricción de buckets públicos configurado"
    ...
    echo "aws s3api get-public-access-block --bucket-name BUCKET_NAME --region $AWS_REGION --profile $AWS_PROFILE"
