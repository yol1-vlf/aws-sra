...
# Script para desplegar AWS SRA Firewall Manager Solution
# Uso: ./deploy-cDeployFirewallManagerSolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-firewall-manager-org-solution"
TEMPLATE_FILE="aws_sra/solutions/firewall_manager/firewall_manager_org/templates/sra-firewall-manager-org-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de Firewall Manager
print_info "Extrayendo parámetros de Firewall Manager..."
...
# Extraer parámetros de Firewall Manager
FIREWALL_MANAGER_PARAMS=$(extract_section_params "rFirewallManagerSolutionStack")

if [ -z "$FIREWALL_MANAGER_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de Firewall Manager del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros extraídos para el despliegue:"
echo "$FIREWALL_MANAGER_PARAMS"

# Convertir los parámetros a formato para CloudFormation deploy
FIREWALL_MANAGER_PARAMS=$(echo "$FIREWALL_MANAGER_PARAMS" | tr '\n' ' ')
...
print_info "Parámetros de Firewall Manager extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-firewall-manager-org-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de Firewall Manager Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $FIREWALL_MANAGER_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de Firewall Manager completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de Firewall Manager implementada:"
    echo "  ✅ Firewall Manager habilitado en la organización"
    echo "  ✅ Administración delegada configurada en la cuenta de auditoría"
    echo "  ✅ Política de Security Groups creada"
    echo "  ✅ Política de WAF creada"
    echo "  ✅ VPC configurada para Security Groups"
    echo "  ✅ Configuración de red interna: 192.168.1.0/24"
    echo "  ✅ CIDR de VPC: 10.0.0.0/28"
    echo "  ✅ Roles IAM configurados para la gestión"
    echo "  ✅ Logs de Lambda configurados"
    echo "  ✅ Remediation automática: deshabilitada"
    echo "  ✅ Gestión centralizada de políticas de seguridad"
    ...
    echo ""
    print_info "Para verificar la configuración de Firewall Manager:"
    echo "aws fms get-admin-account --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws fms list-policies --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws fms list-security-group-policies --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws fms list-waf-policies --region $AWS_REGION --profile $AWS_PROFILE"
    
    echo ""
    print_info "Para verificar el estado de las políticas:"
    echo "aws fms get-policy --policy-id <policy-id> --region $AWS_REGION --profile $AWS_PROFILE"
    echo "aws fms list-compliance-status --policy-id <policy-id> --region $AWS_REGION --profile $AWS_PROFILE"
