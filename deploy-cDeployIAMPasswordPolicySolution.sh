...
# Script para desplegar AWS SRA IAM Password Policy Solution
# Uso: ./deploy-cDeployIAMPasswordPolicySolution.sh [AWS_PROFILE] [PARAMETERS_FILE]
...
STACK_NAME="sra-iam-password-policy-solution"
TEMPLATE_FILE="aws_sra/solutions/iam/iam_password_policy/templates/sra-iam-password-policy-main-ssm.yaml"

print_info "Stack: $STACK_NAME"
print_info "Template: $TEMPLATE_FILE"

# Extraer parámetros específicos de IAM Password Policy
print_info "Extrayendo parámetros de IAM Password Policy..."
...
# Extraer parámetros comunes (que incluyen los de IAM Password Policy)
COMMON_PARAMS=$(extract_section_params "rIAMPasswordPolicySolutionStack")

if [ -z "$COMMON_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros comunes del archivo $PARAMETERS_FILE"
    exit 1
fi
...
IAM_PASSWORD_POLICY_PARAMS=$(echo "$COMMON_PARAMS" | tr '\n' ' ')

if [ -z "$IAM_PASSWORD_POLICY_PARAMS" ]; then
    print_error "No se pudieron extraer parámetros de IAM Password Policy del archivo $PARAMETERS_FILE"
    exit 1
fi

print_info "Parámetros de IAM Password Policy extraídos exitosamente"
...
if [ ! -f "$TEMPLATE_FILE" ]; then
    ...
    find . -name "sra-iam-password-policy-main-ssm.yaml" 2>/dev/null || print_warning "No se encontró el archivo de template"
    exit 1
fi
...
# Realizar deploy
print_info "Iniciando despliegue de IAM Password Policy Solution..."

aws_cmd="aws cloudformation deploy"
aws_cmd+=" --template-file $TEMPLATE_FILE"
aws_cmd+=" --stack-name $STACK_NAME"
aws_cmd+=" --parameter-overrides $IAM_PASSWORD_POLICY_PARAMS"
...
if [ "$FINAL_STATUS" = "CREATE_COMPLETE" ] || [ "$FINAL_STATUS" = "UPDATE_COMPLETE" ]; then
    print_info "✅ Despliegue de IAM Password Policy completado exitosamente!"
    print_info "Stack: $STACK_NAME"
    ...
    echo ""
    print_info "Configuración de IAM Password Policy implementada:"
    echo "  ✅ Política de contraseñas configurada"
    echo "  ✅ Parámetros de seguridad aplicados"
    ...
    echo ""
    print_info "Para verificar la política de contraseñas:"
    echo "aws iam get-account-password-policy --region $AWS_REGION --profile $AWS_PROFILE"
